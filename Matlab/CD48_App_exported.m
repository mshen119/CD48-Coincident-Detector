...
%Last revision: 9/26/2020
    ...
    methods (Access = private)
        ...
        function updateGuiTimer(app)
            ...
            app.TimeBinEdges = [app.TimeBinEdges (app.TimeBinEdges(end)+(app.RepeatPeriodEditField.Value)/1000)];
            
            %stop the timer once it reached desired runtime
            while app.RuntimeValue.Value/1000 < app.TimeBinEdges(end)
                TurnOffTimer(app);
                return;
            end
            ...
            while app.overflow_status ==1
                TurnOffTimer(app);
                app.OverflowClearButton.Value = true;
                uialert(app.UIFigure, 'Detected overflow flag. Clear Overflow and Try again', 'Overflow Warning');
                return;
            end
        ...
        function CalculateCountRate(app)
            ...
            if app.TimeBinEdges(end) == 0
                ...
                repeatTime = app.RepeatPeriodEditField.Value/1000;
                ...
                app.countrate(1) = sum1/(m*repeatTime);
                app.countrate(2) = sum2/(m*repeatTime);
                app.countrate(3) = sum3/(m*repeatTime);
                app.countrate(4) = sum4/(m*repeatTime);
                app.countrate(5) = sum5/(m*repeatTime);
                app.countrate(6) = sum6/(m*repeatTime);
                app.countrate(7) = sum7/(m*repeatTime);
                app.countrate(8) = sum8/(m*repeatTime);
    ...
    methods (Access = private)
        ...
        function ResetButtonPushed(app, event)
            ...
            %repeat period back to 1000ms
            app.RepeatPeriodEditField.Value = 1000;
            
            %trigger level back to 2.0V
            app.TriggerlevelVSlider.Value = 2.0;
            
            %runtime of 10000
            app.RuntimeValue.Value = 10000;
        ...
        function StartPlotButtonPushed(app, event)
            ...
            %turn on toggle
                ...
                period = (app.RepeatPeriodEditField.Value)/1000;
                set(app.myTimer, "Period", period);
                set(app.myTimer, "StartDelay", period)
                start(app.myTimer);
    ...
    methods (Access = private)
        ...
        function createComponents(app)
            ...
            app.TriggerlevelVSlider.Value = 2;
            ...
            app.PossiblebugsanddebuggingTextArea.Value = {'READ ME'; 'Setup info and debugging:'; ''; '1. If the app window appears to be too big, check Computer/Settings/Display Settings. Recommend 100%'; ' '; '2.  Steps to connect:'; '    i)   Find port number with Start/Devices Manager '; '    ii)  Open PORT => Check the Port'; '    iii) Enter onto COM port # above'; ''; '3. If runtime < repeat time period, it will be set to 3600000'; '4. Maximum repeat period is 65000 ms'; '5. Minimum repeat period is 100 ms'; ''; '6. Possible error writing to serial port: '; '      - Close all running Matlab program.'; '      - Unplug USB cable'; '      - Reconnect the USB'; '      - Open the program and try again'; ''; ''};
